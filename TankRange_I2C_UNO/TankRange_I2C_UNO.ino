...
*/
#define SDA_PIN 4              //Default SDA is Pin4 PORTC for the UNO -- you can set this to any tristate pin
...
void read_the_sensor_example(){
  ...
  if (error){                  //If you had an error starting the sensor there is little point in reading it as you will get old data.
    ...
    delay(100);
    range = read_sensor(225);   //reading the sensor will return an integer value -- if this value is 0 there was an error
    Serial.print("R:");
...
void address_polling_example(){
  ...
  //    return two bytes.
  for (byte i=100; i!=0; i+=2){   //start at 2 and count up by 2 until wrapping to 0. Checks all addresses (2-254) except 0 (which cannot be used by a device)
    error = 0;
